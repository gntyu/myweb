(function webpackUniversalModuleDefinition(root, factory) {
	if (typeof exports === 'object' && typeof module === 'object') module.exports = factory(require("echarts"));
	else if (typeof define === 'function' && define.amd) define(["echarts"], factory);
	else if (typeof exports === 'object') exports["echarts-liquidfill"] = factory(require("echarts"));
	else root["echarts-liquidfill"] = factory(root["echarts"]);
		...
		echarts.extendChartView({
			...
			render: function(seriesModel, ecModel, api) {
				...
				function getText(waves) {
					...
					insideTextRect.style.fontSize = textStyle.get('fontSize');
					outsideTextRect.style.fontSize = textStyle.get('fontSize');

					// console.log('outsideTextRect',outsideTextRect)
					// console.log('insideTextRect',insideTextRect)
	...
	function(module, exports, __webpack_require__) {
		...
		function animateOrSetProps(isUpdate, el, props, animatableModel, dataIndex, cb) {
			...
			animatableModel.ifEnableAnimation ? animatableModel.ifEnableAnimation()
	...
	function(module, exports, __webpack_require__) {
		...
		function Displayable(opts) {
			...
			for (var name in opts) {
				if (
				opts.hasOwnProperty(name) && name !== 'style') {
					this[name] = opts[name];
				}
			}
	...
	function(module, exports, __webpack_require__) {
		...
		function createTrackClip(animator, easing, oneTrackDone, keyframes, propName) {
			...
			var arrDim = (
			isValueArray && isArrayLike(firstVal[0])) ? 2 : 1;
			var trackMaxTime;
		...
		function parse(colorStr) {
			...
			if (str.charAt(0) === '#') {
				if (str.length === 4) {
					...
					return [
					((iv & 0xf00) >> 4) | ((iv & 0xf00) >> 8), (iv & 0xf0) | ((iv & 0xf0) >> 4), (iv & 0xf) | ((iv & 0xf) << 4),
					1];
	...
	function(module, exports, __webpack_require__) {
		...
		function someVectorAt(shape, t, isTangent) {
			...
			if (cpx2 === null || cpy2 === null) {
				return [
				(isTangent ? cubicDerivativeAt : cubicAt)(shape.x1, shape.cpx1, shape.cpx2, shape.x2, t), (isTangent ? cubicDerivativeAt : cubicAt)(shape.y1, shape.cpy1, shape.cpy2, shape.y2, t)];
			} else {
				return [
				(isTangent ? quadraticDerivativeAt : quadraticAt)(shape.x1, shape.cpx1, shape.x2, t), (isTangent ? quadraticDerivativeAt : quadraticAt)(shape.y1, shape.cpy1, shape.y2, t)];
			}
		}
		module.exports = __webpack_require__(10).extend({
	...
	function(module, exports, __webpack_require__) {
		...
		function getWaterPositions(x, stage, waveLength, amplitude) {
			if (stage === 0) {
				return [
				[x + 1 / 2 * waveLength / Math.PI / 2, amplitude / 2], [x + 1 / 2 * waveLength / Math.PI, amplitude], [x + waveLength / 4, amplitude]];
			} else if (stage === 1) {
				return [
				[x + 1 / 2 * waveLength / Math.PI / 2 * (Math.PI - 2),
				amplitude], [x + 1 / 2 * waveLength / Math.PI / 2 * (Math.PI - 1),
				amplitude / 2], [x + waveLength / 4, 0]]
			} else if (stage === 2) {
				return [
				[x + 1 / 2 * waveLength / Math.PI / 2, -amplitude / 2], [x + 1 / 2 * waveLength / Math.PI, -amplitude], [x + waveLength / 4, -amplitude]]
			} else {
				return [
				[x + 1 / 2 * waveLength / Math.PI / 2 * (Math.PI - 2), -amplitude], [x + 1 / 2 * waveLength / Math.PI / 2 * (Math.PI - 1), -amplitude / 2], [x + waveLength / 4, 0]]
			}
		}
